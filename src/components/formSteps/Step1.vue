    ...
    <section class="card-wrapper">
        <card>
            <section class="form-container step1" aria-label="step1-form">
                <h2>Personal Info</h2>
                <p class="form-cotainer__description">Please provide your name, email address, and phone number.</p>
                <section class="form-inputs step1" aria-label="step1-inputs">
                    <label for="name">Name <span class="error-message"
                            :class="{ 'error-message--active': nextStepClicked && !userChoise.name }">{{ errorMessage
                            }}</span></label>
                    <input type="text" 
                            name="name" 
                    ...
                    <label for="email">Email Address <span class="error-message"
                            :class="{ 'error-message--active': nextStepClicked && !userChoise.email }">{{ errorMessage
                            }}</span></label>
                    ...
                    <input type="email" pattern="[^@\s][^@\s]+@[^@\s][^@\s]+\.[^@\s]{2,64}" name="email"
                        title="e.g. stephenking@lorem.com at least 2 char for each part"
                        placeholder="e.g. stephenking@lorem.com" v-model="userChoise.email" aria-invalid="true"
                        aria-required="true" :class="{'input--error': nextStepClicked && !userChoise.email }">
                    <label for="phone">Phone Number <span class="error-message"
                            :class="{ 'error-message--active': nextStepClicked && !userChoise.phoneNumber }">{{
                            errorMessage
                            }}</span></label>
                    <input type="number" name="phone" placeholder="e.g. +1 234 567 890" v-model="userChoise.phoneNumber"
                        aria-invalid="true" aria-required="true"
                        :class="{ 'input--error': nextStepClicked && !userChoise.phoneNumber }">
                </section>
            </section>
        </card>
    </section>
