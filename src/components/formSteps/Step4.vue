      ...
      <section class="form-container step3" aria-label="step3-form">
        ...
        <section class="last-check__wrapper">
          <div class="last-check__plan">
            <div class="last-check__info">
              <h3>
                {{
                  `${formStore.userPlanChosen.plan.name} (${monthlyOrYearly.monthlyYearly})`
                }}
              </h3>
              ...
              <p
                role="button"
                tabindex="0"
                @click="emit('changePlanAction')"
                @keydown="emit('changePlanAction')"
              >
                Change
              </p>
            </div>
            <p class="last-check__plan-price total-price-plan">
              {{ `$${monthlyOrYearlyPrice}/${monthlyOrYearly.moOrYr}` }}
            </p>
          </div>
          <div v-for="addOn in filteredAddOns" class="last-check__services">
            <p class="last-check__service-title">
              {{ `${addOn.name} (${monthlyOrYearly.moOrYr})` }}
            </p>
            <p class="last-check__service-price">
              {{
                isMonthly
                  ? `+$${addOn.price}/mo`
        ...
        <section class="last-check__total">
          <p class="total__title">
            Total {{ `(${monthlyOrYearly.perMonthPerYear})` }}
          </p>
          <p class="total__price">
            {{ `+$${totalPrice}/${monthlyOrYearly.moOrYr}` }}
          </p>
        </section>
      </section>
    </card>
